...
<br><br>
不過失配時用fail跳到新節點之後還是要再配對一次子節點, 這裡會產生停頓, 而這時可以針對這部分改善
...
<br>, 這樣字典樹就變成字典圖, 進行在配對過程中就算失配或者配對結束時都可以跟據主字串的下一個字符直接跳到合適的分支或者根繼續配對,沒有任何停頓。
<br>, fail指針只拿來獲取後綴。
<br>需要留意的是這項優化在搆建transfer map的時候會要求fail指針指向的上層節點已搆建好fail指針和transfer map, 所以搆建時要用廣度優先搜索
